...
<%
	...
	int maxSize = 5 * 1024 * 1024; //최대 업로드 될 최대 크기 5MB
	...
	if(rs.next()){
		if(fileName != null){
			sql = "update product set p_name=?, p_unitPrcie=?, p_description=?, p_manufacturer=?, p_category=?, p_unitsInStock=?, p_condition=? p_fileName=? where p_id=?";
			pstmt = conn.prepareStatement(sql);
			...
			sql = "update product set p_name=?, p_unitPrcie=?, p_description=?,"
